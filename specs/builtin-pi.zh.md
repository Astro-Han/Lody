...
导入绑定到该机器上唯一的同类型 Provider。列举和加载使用该 Provider 的启动设置，新导入和
之前导入的会话都会记录它，因此继续对话时应用它的环境变量、运行时覆盖和 Pi 扩展，输入框也
提供它的模型与思考强度选择。没有或有多个同类型 Provider 时，会话保持未绑定，继续对话使用
该类型的默认启动。已有的绑定不会被替换。此规则适用于所有 Provider 的历史导入，不限于 Pi。
