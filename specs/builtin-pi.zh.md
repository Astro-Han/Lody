...
导入的会话绑定到该机器上唯一的同类型 Provider，列举和加载都使用该 Provider 的启动设置；之前的
导入在其项目下次同步时绑定。没有或有多个同类型 Provider 时保持未绑定，已有绑定不会被替换。每次
导入写入都把 `session/load` 报告的模型、模式和选项记为最后一条导入用户消息的运行时选择；更新的
Lody 消息保留自己的选择。记录的模型若不在绑定 Provider 的目录中，仍退回目录默认值。此规则适用于
所有 Provider 的历史导入；Codex 的只读历史方法不报告选择。
