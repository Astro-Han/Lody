...
导入的 Pi 会话没有模型和思考强度选项。所有 Provider 的导入都不写 `agentConfigId`，输入框找不到
Provider 的能力缓存，只能退回静态目录，而 Pi 没有静态目录；继续对话也不会应用该 Provider 的环境
变量、运行时覆盖和扩展。现在导入通过该机器上唯一的同类型 Provider 列举、加载并绑定，列举时为之前
的导入补上绑定。没有或有多个时保持未绑定：在多行之间猜测，可能把原生会话换到另一个账号或端点。
列举与继续使用同一个 Provider，Provider 自设的数据目录在导入和继续之间保持一致。模型列表仍依赖
该 Provider 的能力缓存。
